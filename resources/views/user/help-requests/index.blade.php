...
@section('content')
    ...
    <div class="row mb-5">
        ...
        <x-dashboard-card title="Créer une demande" icon="bi-plus-circle-fill"
            iconBgClass="bg-success bg-opacity-10 text-success" buttonText="Créer" :buttonUrl="route('user.help-requests.create')" buttonClass="success" />

        @if ($acceptedRequests->isNotEmpty())
            <x-dashboard-card title="Mes demandes acceptées" :count="$acceptedRequests->count()" icon="bi-check2-circle"
                iconBgClass="bg-primary bg-opacity-10 text-primary" buttonText="Voir" buttonUrl="#accepted-requests"
                buttonClass="primary" />
        @endif
    </div>
    ...
    <div class="container mt-5" id="accepted-requests">
        <x-dashboard-section-title type="greenRequests">Mes demandes acceptées</x-dashboard-section-title>
        @if ($acceptedRequests->isNotEmpty())
